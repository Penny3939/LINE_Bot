...
def index():
    if request.method == 'GET':
        return 'ok'
    body = request.json    #外部進來的參數存成json
    events = body["events"]
    if request.method == 'POST' and len(events) == 0: #len(events)==0,在verify webhook的時候會出現success
        return 'ok'
    print(body)
    if "replyToken" in events[0]:
        ...
        if events[0]["type"] == "message":
            if events[0]["message"]["type"] == "text": #如果是文字text的話
...
def getNameEmojiMessage():
    ...
    #import string
    #上方A~Z大小寫可寫成: lookUpstr= string.ascii_uppercase + string.ascii_lowercase 
    productId = "5ac21a8c040ab15980c9b43f"
    name = "Penny"
    emojis = list()
    text = "$" *len(name) #有幾個英文字就有幾個$符號
    for index,char in enumerate(name): #若name=Penny, 會印出index0=P,1=>e,2=>n...
        emojis.append({
            "index":index,
            "productId":productId,
            "emojiId":str(lookUpStr.index(char)+1).zfill(3)
            #查PENNY字母是出現在Lookupstr字串裡的第幾個(從1開始排序,故要+1)
            #zfill 因規則要三個數字,ex:016,若數字不足補零
        })  
...
def getCallCarMessage(data):
    message = {
        ...
        "template": {
            "type": "buttons",
            "text": f"Select date to {data['title']} pick up",
            "actions": [
            {
            "type": "datetimepicker",
            "label": "預約",
...
def getTaipei101LocationMessage():
    ...
    message['address']= "No. 45, City Hall Rd, Xinyi District, Taipei City, 110"
    message['latitude']= 25.0330304
    message['longitude']= 121.5579962
    return message


def getMRTVideoMessage():
    originalContentUrl_02= F"{end_point}/static/taipei_101.jpeg"
    message = dict()
    message['type']= "video"
...
def getImageMessage(originalContentUrl):
    message = dict()
    message["type"] = "image"
    message["originalContentUrl"] = originalContentUrl #是Line跟local server URL做請求,不是Line Server
...
def line_login():
    if request.method == 'GET':
        ...
        if code and state:
            ...
            # statusMessage = content["statusMessage"] 跟上方結果一樣,但若line狀態那沒有文字會出錯
            print(content)
